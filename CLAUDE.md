...
**Proposed next step:** deploy `deployfleet_ai_agents` (Phase 5) and `deployfleet_demo_zm` to the demo server — `deployfleet_demo_zm` depends on all 42 other modules, so a single `-i deployfleet_ai_agents,deployfleet_demo_zm` (or two sequential installs) should be sufficient once the demo database's existing modules are updated. Watch the install log for the usual nightly-build version-drift class of bug before assuming the dataset generated cleanly.
