...
**Done:** the Phase 1 architecture analysis (`docs/architecture/01` through `15`), this operating guide, the supporting root-level docs listed in §2, and formal sign-off resolving 7 of the 8 hard risks in [06-risks-and-recommendations.md](docs/architecture/06-risks-and-recommendations.md). **Phase 0** (`deployfleet_core`, `deployfleet_security`, `deployfleet_event_bus`, `deployfleet_ai_core`), **Phase 1 — Operational Foundation** (`deployfleet_driver`, `deployfleet_vehicle`, `deployfleet_customer`, `deployfleet_route`, `deployfleet_dispatch`, `deployfleet_trip`, `deployfleet_delivery`), **Phase 2 — Cost Control** (`deployfleet_freight_calculator`, `deployfleet_load_expense`, `deployfleet_driver_advance`, `deployfleet_fuel`, `deployfleet_parts`, `deployfleet_tyres`, `deployfleet_workshop`, `deployfleet_maintenance`, `deployfleet_assets`), **Phase 3 — Compliance** (`deployfleet_compliance`, `deployfleet_vehicle_compliance`, `deployfleet_dispatch_compliance`, `deployfleet_insurance`, `deployfleet_payroll`, `deployfleet_l10n_zm`, `deployfleet_leave`, `deployfleet_loans`, `deployfleet_driver_performance`), and **Phase 4 — Customer Platform** (`deployfleet_billing`, `deployfleet_accounting`, `deployfleet_zra`, `deployfleet_client_reports`, `deployfleet_customer_portal`, `deployfleet_notifications`, `deployfleet_mobile_dispatcher`, `deployfleet_mobile_customer`, `deployfleet_mobile_push_bridge`, `deployfleet_ai_permissions`, `deployfleet_ai_actions`, `deployfleet_ai_whatsapp`) are all implemented, tested, lint-clean, and committed — 41 modules total. The Phase 3 AI Compliance Agent itself is not part of this — that's [08-ai-architecture.md](docs/architecture/08-ai-architecture.md) agent-catalog work, not yet started.

Phase 4 notes worth carrying forward, not re-discovering later: `deployfleet_zra`'s endpoint paths/field names come from a community Postman collection, not ZRA's own spec PDF (which 503'd while building it) — verify against ZRA's actual sandbox before any real submission, and get someone who knows ZRA's VAT category rules to review the default VAT category code. `deployfleet_ai_whatsapp`'s webhook does not yet verify Meta's `X-Hub-Signature-256` signature — not hardened against spoofed requests until that's closed. Both are flagged in their own module READMEs, not just here.

**A live demo is deployed and running** on the shared demo server at `http://199.192.23.46:4169/`, alongside — not disrupting — that server's pre-existing production and staging Odoo instances, per the isolated stack in [DEPLOYMENT.md](DEPLOYMENT.md). All 29 Phase 0–3 modules are installed on the demo database; the 12 Phase 4 modules built this session are not yet deployed there. DeepSeek is configured as the active AI provider via the Odoo backend (`deployfleet.ai.config`), not via any file in this repository.

The demo server runs an Odoo 19 nightly snapshot (`19.0-20260723`), not a stable release — this has already caused two version-drift bugs during Phase 2/3 install (`res.groups.category_id` removed in favor of `privilege_id`; the base settings view's `div.settings` xpath anchor changed). Pinning to a stable Odoo 19 release once one is cut remains an open recommendation, not yet decided.
...
**Proposed next step:** deploy the 12 new Phase 4 modules to the demo server (same install-then-verify loop used for Phase 2/3, watching for the same class of Odoo-nightly version-drift bugs), then Phase 5 — Intelligence from [05-implementation-roadmap.md](docs/architecture/05-implementation-roadmap.md).
