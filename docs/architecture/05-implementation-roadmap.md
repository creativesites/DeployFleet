...
- **Candidate Phase 6+, vision only, not yet scheduled**: [10-lead-intelligence-architecture.md](10-lead-intelligence-architecture.md) (AI-powered lead capture/sales automation) and [11-dispatch-network-architecture.md](11-dispatch-network-architecture.md) (dispatch as a third-party/broker business model, distinct from the internal-fleet `deployfleet_dispatch` built in Phase 1). Both are new commercial surface area for different user types than the current MVP targets — evaluate only after Phases 1–5 are proven with the real customer already onboarding, per each document's own sequencing note.
