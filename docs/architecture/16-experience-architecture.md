...
**Updated in Phase D / Slice 1:** the expanded detail now also surfaces an `AiRecommendationCard` (§6/§18) built from `deployfleet.maintenance.prediction` — the vehicle's latest predicted risk score/level/basis, silent when risk is "low" — the first real, working instance of this document's "ambient AI on this screen" concept anywhere in `deployfleet_ui`, see §7.16 for the full Copilot Console slice this shipped alongside.
...
**Current state (Phase D / Slice 1 — the Copilot Console destination screen is built):** a real command-center-class screen (`static/src/copilot_console/copilot_console.js`), light Command Layer glass per the mid-Phase-D retrofit. Two sections: an **Agent Catalog** (all six `deployfleet.ai.agent` personas — name, description, related models, model tier, data category — each with a real, working Enable/Disable toggle wired to `deployfleet.ai.feature.enabled`) and a **Usage & Cost Dashboard** over `deployfleet.ai.usage` (this month's cost/tokens via the model's own `total_cost_this_month()`, an all-time cache-hit rate, a per-feature cost/token breakdown via `read_group`, and a recent-calls log). **The natural-language question interface is also built**, scoped per-agent rather than as one generic global query box: each agent card has a real "Ask" box that calls `deployfleet.ai.core.complete(feature.key, agent.system_prompt_template, question)` — the same mandatory-pipeline entry point (policy/permission/budget checks, response cache, provider call, usage logging) every other AI feature goes through, not a shortcut around it. **The AI Recommendation Card is also built** (a genuinely new atom, `DeployfleetAiRecommendationCard`) with its first real consumer wired into the Fleet Command Center's expanded vehicle detail, surfacing `deployfleet.maintenance.prediction`'s risk score/level/basis inline — silent when risk is "low," the same silent-unless-actionable discipline as Mission Control's attention strip.

**What's still deferred, not silently dropped:** per-record contextual awareness for the Copilot Rail/Console (neither yet knows which record the user has open elsewhere in the app — doc 16's original "contextually relevant agent output for whatever record is open" needs a safe, verified hook into the action manager's current controller, deliberately not attempted speculatively against an unverified internal API in this exact Odoo 19 nightly, the same risk-aversion reasoning that kept the Launcher off a `web.NavBar` patch); AI Recommendation Cards on the remaining flagship screens (Dispatch Board, Billing — Fleet Command Center is the first, not the last); and per-agent "recent output"/"confidence" in the catalog (the four Phase 5 prediction models each use a different schema for their own risk metric, and unifying that needs per-agent custom rendering logic, not a generic read).
...
The **Copilot Console** (§7.16) sits deliberately at the Phase C/D boundary: its ambient half (the Copilot Rail, AI Recommendation Cards appearing inline on flagship screens) belongs in Phase C alongside the screens it annotates — **the Copilot Rail's ambient half, specifically its live approval queue, is now built** (§7.16); its destination half (the full agent-catalog-and-cost-dashboard console) trailed into what this document and CLAUDE.md track as "Phase D / Slice 1" and **is now built** — the agent catalog, usage/cost dashboard, natural-language query interface, and the AI Recommendation Card's first real consumer (Fleet Command Center) all shipped (§7.16). **Naming note to avoid confusion:** this "Phase D" (the Copilot Console's own build slice) is distinct from this section's own "Phase D — Operational workspaces" bucket immediately above (Maintenance/Billing/Driver/Customer/Compliance/Workshop) — the Copilot Console work was tracked under the "Phase D" label because §7.16 itself first used that word to describe where its destination half could trail to, not because it belongs to the operational-workspaces bucket.
