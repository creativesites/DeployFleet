...
**Reuse for DeployFleet:** directly, as the **DeployFleet Launcher**. Two specific ideas are worth preserving deliberately rather than being lost in a rebuild: the **priority-workspace strip with keyboard shortcuts** (translate to `Alt+D` Dispatch Board, `Alt+F` Fleet Command Center, `Alt+M` Maintenance Intelligence, `Alt+B` Billing Workspace, `Alt+C` Copilot Console, `Alt+H` Mission Control) and the **keyword-to-gradient color-family mapping** for auto-coloring app tiles, extended with the AI-content violet family (doc 17) for anything Copilot-related in the grid. New for DeployFleet: **persist recents and pin favorites per user** — a genuine gap in the source component, and a real click-reduction win for a dispatcher who opens the same four screens every morning.
...
**Current state:** does not exist — opening the app lands on whatever the last-used menu was, or Odoo's default. **Problems:** no orientation moment at all; a manager has no single place to ask "is anything wrong today," and the first thing any new user sees today is a model-name menu list, the ERP first impression §1/§2.5 flags directly. **Missing dashboards:** entirely missing. **Redesign direction:** Mission Control per §5/§2.5 — moving trucks, today's numbers, and the attention strip, designed explicitly as the "wow" first-five-minutes moment, not a generic landing page.
...
**Phase C — Flagship experiences.** Fleet Command Center (§7.10), Dispatch Board (§7.9), Mission Control (§5/§7.2), and the Copilot Rail (§6/§8) — the screens a demo lives or dies by, and the ones that establish the signature-widget visual language (Truck Health Ring, AI Recommendation Card) the remaining workspaces will reuse.
...
The **Copilot Console** (§7.16) sits deliberately at the Phase C/D boundary: its ambient half (the Copilot Rail, AI Recommendation Cards appearing inline on flagship screens) belongs in Phase C alongside the screens it annotates; its destination half (the full approval-queue-and-cost-dashboard console) can trail into Phase D without blocking the flagship screens' launch, since ambient AI delivers value before the dedicated console exists to manage it in bulk.
