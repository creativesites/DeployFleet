...
- **Command Layer glass recipe**: dark scrim (`rgba` of a near-black, ~0.6–0.76 opacity) + heavy blur (24–28px) + a slight saturation boost on whatever's behind it, behind a translucent gradient card with a hairline border and a subtle inset top highlight — the Enterprise OS Launcher's actual recipe (doc 16 §3.2), inherited because it's genuinely well executed, restyled in DeployFleet's teal/amber palette rather than DeployGuard's navy.
