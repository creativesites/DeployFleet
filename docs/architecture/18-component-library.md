...
**Implementation note on the AI Recommendation Card (Phase D):** built as `DeployfleetAiRecommendationCard`, its first real consumer wired into the Fleet Command Center's expanded vehicle detail, surfacing `deployfleet.maintenance.prediction`'s risk score/level/basis (silent when `risk_level` is "low"). One deliberate deviation from this table's "exactly two actions (Accept / Dismiss)": a predictive-maintenance risk score has no single concrete "accept" write the way a Dispatch Board suggestion does (confirm this driver/vehicle pairing) — there is nothing to *accept*, only something to *act on* or *set aside*. The shipped props are `actionLabel`/`onAction` (an open-ended caller-supplied action — here, "Open full record") and `onDismiss`, not a hardcoded Accept/Dismiss pair, so a future consumer with a genuine accept-able suggestion (e.g. an AI-proposed dispatch swap) can still route that action through the mandatory suggestion→approval→execute→audit pipeline via its own `onAction` handler, exactly as this table requires — the component itself never writes anything.
