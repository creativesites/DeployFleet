...
|---|---|---|
| **1 — Read-only chat** | Chat tab UI, session persistence (§5's two new fields), 5 read-only tools (§3), `useCopilotContext()` (§2), plain-text responses only (no §7/§8 yet) | Nothing new — every piece is additive to already-shipped infrastructure |
