...
**§10 Phase 1 is built** — plain-text, session-persisted, multi-turn Chat in the Copilot Rail. **§10 Phase 1b is built** — §3's provider-native tool-calling (`deployfleet.ai.core.complete_with_tools()`, the `deployfleet.ai.tool` fixed-dispatch registry in `deployfleet_ai_agents`, 5 read-only tools) and §2's `useCopilotContext()` hook (wired into Fleet Command Center and the Dispatch Board, folded into the Chat prompt as a non-persisted `context_note`). **§10 Phase 2 is built** — §7's structured-output contract (a portable trailing ```` ```json {...} ``` ```` fenced block, parsed server-side, rather than each provider's native JSON-mode parameter — a deliberate deviation, see below) and §8's `ChatMessageRenderer` (two component types: `vehicle_card` with its own live ORM re-fetch, `table` rendered from the tool-derived data as given). `deployfleet.ai.chat.message` gained the `tool_calls`/`rich_payload` fields §5 already speced. **§10 Phase 3 is built** — §4's auto-executable action tier: a new `deployfleet.ai.auto.executable.action` allow-list model, `action_method`/`auto_executed` fields on `deployfleet.ai.action.request`, an extended `_execute()` supporting zero-arg `action_*()` method calls (not just field writes — the concrete lever most real dispatch/fleet actions actually are), and a new `propose()` entry point every AI-initiated write should call. First (and so far only) write tool: `mark_vehicle_available`, allow-listed for auto-execution. **§10 Phase 4 is now also built** — §6's context/memory layer: `deployfleet.ai.entity.summary` (a deterministic, Python-computed summary cache — not a second LLM call — with hybrid invalidation: a `source_signature` hash check on every read, plus a real `deployfleet.event.subscription` reacting to `deployfleet.trip.completed`/`deployfleet.dispatch.assigned` to proactively delete a stale vehicle summary) and `deployfleet.ai.company.profile` (a minimal stock admin form, folded into every Chat session's system prompt via a new `_effective_system_prompt()`, computed fresh per turn rather than baked into the session's stored snapshot). `get_vehicle_summary` is the cache's first and so far only real consumer. See CLAUDE.md's AI & Intelligence writeup for the concrete implementation notes and every deliberate deviation from this doc's original sketch (fixed dict-dispatch tool execution instead of dynamic `getattr`; the fenced-JSON-block convention instead of native JSON-mode; the auto-executable tier redesigned around action-method calls, not doc 21's own original Boolean-per-feature sketch; live-provider verification of both tool-calling adapters still outstanding per §11 item 3). **This closes all four phases of doc 21's rollout** — every phase in §10's table is now built, though several items within Phases 2-3 remain deliberately deferred (see those phases' own rows: inline approval cards, `actions_available` button rendering) and §11's open questions are not all resolved.
...
2. **Never** a feature whose `data_category` is `payroll` or `financial` (`deployfleet.ai.policy`'s existing category blocks, doc 08 §9) — the same category boundary that already keeps AI off payroll/financial data by default stays load-bearing here with zero exception, re-checked on every `propose()` call rather than trusted once at data-entry time.
...
4. **Built as implemented, a deviation from this section's original sketch:** a per-feature `auto_executable` Boolean turned out too coarse to satisfy criterion 3's per-action-type reversibility requirement, and this tier's own concrete examples (`action_set_available()`, `action_confirm()`) are method calls, not field writes — something the pre-existing `_execute()` couldn't do at all before this phase. Replaced with a `deployfleet.ai.auto.executable.action` allow-list model keyed on the exact triple `(action_type, target_model, action_method)`, checked by a new `propose()` classmethod that every write tool calls instead of hand-rolling `create()` + `action_submit_for_approval()`. `action_method` is required on this allow-list model — a bare create/write auto-exec entry isn't offered at all, since this tier's reversibility argument is really about the specific method, not a generic field write.
...
├── summary_text            (Text — built, as implemented)
├── computed_date
├── source_signature        (hash of the inputs used, so a re-fetch can cheaply detect "nothing changed")
```

**As implemented, one deviation from this section's original sketch:** `summary_text` is a deterministic, plain Python-formatted string (`get_vehicle_summary`'s handler builds it from the same fields it already reads), not LLM-generated — a second `complete()`/`complete_with_tools()` call per cache-miss would double the cost/latency of every tool call that uses it, for no clear benefit over ordinary string formatting over data the caller already has.
...
**Invalidation via the event bus (§0), not a cron sweep or a TTL guess — a hybrid with the signature check above, as implemented.** `source_signature` is the general-case mechanism (checked on every read, since not every field this cache might ever summarize publishes a bus event today); one real `deployfleet.event.subscription` row is layered on top for vehicle summaries specifically, naming `deployfleet.ai.entity.summary`/`_handle_bus_event` and subscribed to the two event names `deployfleet_dispatch`/`deployfleet_trip` actually publish that touch a vehicle's status — `deployfleet.trip.completed` and `deployfleet.dispatch.assigned` (both payloads already carry `vehicle_id`) — rather than the broader `vehicle.status.*`/`maintenance.job_card.closed`/`fuel.log.created` patterns this section originally sketched, none of which are real published event names in this codebase. `deployfleet_ai_core` needs zero new manifest dependency to subscribe — event-bus subscriptions reference an event-name string and a model/method name string, never an `ir.model` reference to the publishing module.

**Explicitly not built:** long-term cross-session "extracted intelligence" (the vision brief's "customer usually ships weekly" style pattern-mining) — that's a genuinely open-ended data-mining feature with no clear v1 shape yet, correctly sequenced last the same way doc 08 §11 sequenced predictive maintenance after rule-based maintenance shipped and accumulated real data first.
...
| **2 — Structured responses + rich components** | **Built.** A portable trailing ```` ```json {...} ``` ```` fenced block (`deployfleet.ai.chat.session._extract_rich_payload()`) instead of §7's literal `response_format="structured"` native JSON-mode parameter — works identically across all three providers with zero adapter-specific request-shape changes, at the cost of relying on the model following a prompt instruction rather than a hard API guarantee; degrades to plain text on any parse failure. `ChatMessageRenderer` (§8) ships 2 of the sketched component types — `vehicle_card` (live ORM re-fetch, never trusts LLM-echoed fields) and `table` (rendered from the tool-derived data as given) — driver/shipment cards not yet added, same pattern extends trivially when needed. `actions_available` is parsed and stored in `rich_payload` but not yet rendered as clickable buttons — that's Phase 3's job, since there's no auto-executable/approval action to wire a click to until then. | Phase 1's tool-calling plumbing — done |
| **3 — Action execution** | **Built.** The auto-executable tier (§4, as actually implemented — see that section's item 4): `deployfleet.ai.auto.executable.action`, `action_method`/`auto_executed` fields, `propose()`, and one starting write tool (`mark_vehicle_available`, allow-listed). **Not yet built**: inline approval cards for pending-approval actions surfaced directly in the Chat bubble (today a pending write's outcome is reported back to the user in the tool's own JSON result text - "submitted for manager approval" - not rendered as a clickable card; the Copilot Rail's existing Approvals tab is still the only place to actually approve/reject one). `actions_available` (§7) is parsed and stored but not yet rendered as buttons for the same reason - deferred until a second write tool exists to make the "auto vs. approval" button distinction concretely visible. | Phases 1–2 - done |
| **4 — Memory/context layer** | **Built.** `deployfleet.ai.entity.summary`/`.company.profile` (§6, as actually implemented — see that section), event-bus-driven invalidation for vehicle summaries, `get_vehicle_summary` as the first real consumer, `_effective_system_prompt()` folding the company profile into every Chat turn. Built ahead of "enough real usage to justify it" (this table's own original sequencing note, below) at the user's explicit direction to complete all four phases in one pass, not because usage data arrived first — worth remembering if the cache's actual hit rate turns out low once there's real traffic to check it against. | Phases 1–3 — done |
